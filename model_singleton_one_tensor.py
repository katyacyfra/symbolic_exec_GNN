...
class StateModelEncoder(torch.nn.Module):
    ...
    def convert_to_single_tensor(self, x_dict, edge_index_dict, edge_attr, fixed_size_tensor=False):
        ...
        if fixed_size_tensor:
            ...
            columns_n = max(x_dict['game_vertex'].size()[1], x_dict['state_vertex'].size()[1],
        ...
        row_start = 0
        final_tensor[row_start : self.vertices_n, : x_dict['game_vertex'].size()[1]] = x_dict['game_vertex']
        row_start += self.vertices_n

        self.sizes_dict[('state_vertex')] = (row_start, x_dict['state_vertex'].size()[1])
        final_tensor[row_start : row_start + self.states_n, : x_dict['state_vertex'].size()[1]] = x_dict['state_vertex']
    ...
    def forward(self, x):
        game_x = x[self.sizes_dict[('game_vertex')][0]:self.vertices_n,
        ...
        state_x = x[start: start + self.states_n,
                 :self.sizes_dict[('state_vertex')][1]]
        ...
        parent_index = x[start : start + 2,
                 :self.sizes_dict[('state_vertex', 'parent_of', 'state_vertex')][1]].to(torch.int64)
